...
function dashboard_meetYourCommenters($sidebar_args) {
	...
	echo '<p class="textright"><a href="index.php?page=meetYourCommenters" class="button">'. __('View all') . '</a></p>';
}

// Patch to allow cURL. Thanks to Rick [http://rick.jinlabs.com/]
...
function meetYourCommenters_reports_page() {
	...
	echo "<h1>" . __('Meet your commenters') . "</h1>";
	echo "<p>" . __('Here are listed the profiles and web pages of the commenters that have leave their URL. This is possible thanks to the Google Social Graph API. The profiles are showed because the commenter claims them as its owner on his web linking them with <em>rel="me"</em>. The ones which are with italic font are not reliable and they could not be of the user.') . " Visit <a href=\"http://www.berriart.com/meet-your-commenters/\">http://www.berriart.com/meet-your-commenters/</a> if you have any doubt, comment or sugestion.</p>";	
	...
	if ( $antFrom >= 0 ) {
	echo "<a href=\"index.php?page=meetYourCommenters&amp;from=" . $antFrom . "\">" . __('Previous') . "</a> | ";
	}
	echo "<a href=\"index.php?page=meetYourCommenters&amp;from=" . $posFrom . "\">" . __('Next') . "</a>";
	echo "</p>";
	echo "</div>";
}
...
function meetYourCommenters_print_visitors($comments, $isMYCPage) {
	...
	foreach ( $comments as $comment ) {
		...
		if ($isMYCPage) {
			echo "<li class='visitor-profile-internal'><strong>" . __("Latest posts commented by ") . $comment["name"] . ":</strong></li>";
...
function meetYourCommenters_admin_menu() {
	$hook = add_submenu_page('index.php', __('Meet your commenters'), __('Meet your commenters'), 'manage_options', 'meetYourCommenters', 'meetYourCommenters_reports_page');
}
